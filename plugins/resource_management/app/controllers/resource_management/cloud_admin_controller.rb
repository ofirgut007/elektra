...
module ResourceManagement
  class CloudAdminController < ::ResourceManagement::ApplicationController
    ...
    def show_area(area = nil)
      ...
      @cluster= services_ng.resource_management.find_current_cluster(services: @area_services.map(&:catalog_type))
      @resources = @cluster.resources
      @min_updated_at = @cluster.services.map(&:min_updated_at).min
      @max_updated_at = @cluster.services.map(&:max_updated_at).max
    end
    ...
    def details
      ...
      cluster = services_ng.resource_management.find_current_cluster(services: service_type, resources: resource_name.to_s)
      @cluster_resource = cluster.resources.first or raise ActiveRecord::RecordNotFound, "no data for cluster"
      domains = services_ng.resource_management.list_domains(services: service_type, resources: resource_name.to_s)
